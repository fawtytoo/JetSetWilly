...
static void UpdateDir(int conveyDir)
{
    ...
    if ((System_IsKey(KEY_LEFT) || System_IsJoyLeft() || conveyDir == C_LEFT) && gameMode < GM_RUNNING)
    {
        dir += 1;
    }

    if (System_IsKey(KEY_RIGHT) || System_IsJoyRight() || conveyDir == C_RIGHT || gameMode == GM_RUNNING)
    {
        dir += 2;
    }
    ...
    if ((System_IsKey(KEY_JUMP) || System_IsJoyFire()) && gameMode < GM_RUNNING)
    {
        minerWilly.air = 1;
        minerWilly.jump = 0;
        if (minerWillyRope > 0)
